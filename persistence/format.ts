/**
 * Shared `Result` type and Lua-safe text helpers used by the persistence modules: exact
 * integer text, strict digit parsing, hex framing and the 31-bit checksum.
 *
 * @example
 * ```ts
 * import { hexDecode, hexEncode, integerText } from "@mdlsvensson/wc3-lib/persistence/format";
 *
 * integerText(1093677104); // "1093677104", never "1093677104.0" in Lua
 * hexDecode(hexEncode("W3")); // "W3"
 * ```
 *
 * @module
 */

/** Success with a value, or failure with a short error code. */
export type Result<T> = { ok: true; value: T } | { ok: false; error: string };
/** Builds a failed `Result`. */
export function failure<T>(error: string): Result<T> { return { ok: false, error }; }
/** Whether `value` is an integer in [min, max]. */
export function integer(value: number, min: number, max: number): boolean {
  return value >= min && value <= max && Math.floor(value) === value;
}
/** Whether every character is printable ASCII (32..126). */
...
/** Whether `value` is 1..max characters of letters, digits, `_` or `-`. */
...
/**
 * Parses canonical decimal digits (no sign, no leading zeros) up to `max`.
 * Stops before 32-bit overflow. Returns undefined for anything else.
 */
...
/** Encodes each character code as two lowercase hex digits. */
...
/** Decodes lowercase hex made by `hexEncode`. Returns undefined for odd length, bad digits or non-printable characters. */
