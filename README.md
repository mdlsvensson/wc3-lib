...
A scheduler, script buffs and auras, dummy casters, a damage pipeline, missiles and knockback, save codes and time helpers, for Warcraft III maps written in TypeScript and compiled to Lua with TypeScriptToLua ([wc3.dev-framework](https://github.com/mdlsvensson/wc3-dev-framework)). Published to JSR as `@mdlsvensson/wc3-lib`.
...
The TypeScript sources are what you consume: the map's TypeScriptToLua build compiles them, so only the files reachable from your imports end up in the map. How wc3.dev-framework brings them into a map is documented with the framework.
...
deno task check:docs    # JSDoc on every export, @module docs, examples type-check
