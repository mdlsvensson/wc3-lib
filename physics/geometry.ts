/**
 * Pure vector helpers for the physics systems: points, validation, interpolation,
 * `turnToward` for homing and `segmentSphere` for swept collision.
 *
 * @example
 * ```ts
 * import { turnToward } from "@mdlsvensson/wc3-lib/physics/geometry";
 *
 * // Turn at most 0.1 radians toward +y, keeping the speed of 600.
 * const velocity = turnToward({ x: 600, y: 0, z: 0 }, { x: 0, y: 1, z: 0 }, 0.1);
 * ```
 *
 * @module
 */
...
/** Throws unless `value` is a finite number. `name` goes into the error message. */
